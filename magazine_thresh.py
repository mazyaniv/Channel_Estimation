...
for na,nq,color in resource:
    ...
    if plot_dict['MMSE'] == 1:
        MMSE = [MMSE_func(sigma, sigma, na, nq, matrix_const1, 5000,bound_sim,thresh_space[i],thresh_space[i]) for i in range(len(thresh_space))]
    ...
    if plot_result:
        ...
        for key, (linestyle, marker, data) in plots.items():
            if key in locals():
                plt.plot(thresh_space, data, linestyle=linestyle, marker=marker, label=key+f" $n_a$={na},$n_q$={nq}", color=color)
ax = plt.gca()
...
if save_to_mat:
        ...
        savemat(file_path, {"SNR": thresh_space})
