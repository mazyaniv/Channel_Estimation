...
for na,nq,color in resource:
    ...
    if plot_dict['MMSE'] == 1: #Basically I need more snap for stability
        ...
        MMSE= np.load('MMSE/MMSE,na=1,nq=100,snap=12000,monte=1200.npy')
    ...
    if plot_dict['WBCRB'] == 1:
        if plot_dict['Approx'] == 0:
            WBCRB = [weighted_BCRB(sigma_space[i], sigma_space[i], na, nq, matrix_const1,1000,thresh,thresh) for i in range(len(chosen_space))]
